...
public class OperatorsPresenter extends BasePresenter<OperatorsListener> {

    private static int retryCount = 0;

    public void operatorsList(final Context context, final HashMap<String, RequestBody> requestBody) {

        getView().enableLoadingBar(context, true, context.getString(R.string.txt_please_wait));

        MyApplication.getInstance().getAPIInterface().androidOperators(requestBody)
                .enqueue(new Callback<ResponseOperators>() {
                    @Override
                    public void onResponse(@NonNull Call<ResponseOperators> call, @NonNull Response<ResponseOperators> response) {
                    ...
                    @Override
                    public void onFailure(@NonNull Call<ResponseOperators> call, @NonNull Throwable t) {
                        try {
                            t.printStackTrace();
                        } catch (Exception e) {
                            e.printStackTrace();
                       ...
                       /*     retryCount++;
                            if (retryCount < 3) {
                                ...
                                operatorsList(context, requestBody);
                            } else {
                                getView().onErrorToast(context.getString(R.string.msg_unable_connect_server));
                                getView().enableLoadingBar(context, false, "");
                            }*/
