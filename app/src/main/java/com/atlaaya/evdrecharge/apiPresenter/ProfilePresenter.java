...
public class ProfilePresenter extends BasePresenter<UpdateProfileListener> {

    private static int retryCount = 0;

    public void updateProfile(final Context context, final HashMap<String, RequestBody> requestBody) {

        getView().enableLoadingBar(context, true, context.getString(R.string.txt_please_wait));

        MyApplication.getInstance().getAPIInterface().updateProfile(requestBody)
...
//                            retryCount = 3;
    ...
    public void viewProfile(final Context context, final HashMap<String, RequestBody> requestBody) {

        getView().enableLoadingBar(context, true, context.getString(R.string.txt_please_wait));

        MyApplication.getInstance().getAPIInterface().viewProfile(requestBody)
                .enqueue(new Callback<ResponseLogin>() {
                    @Override
                    public void onResponse(@NonNull Call<ResponseLogin> call, @NonNull Response<ResponseLogin> response) {
                        getView().enableLoadingBar(context, false, "");
                    ...
                    public void onFailure(@NonNull Call<ResponseLogin> call, @NonNull Throwable t) {
                       ...
                       /*     retryCount++;
                            if (retryCount < 3) {
                                getView().onErrorToast(context.getString(R.string.msg_internet_seems_slow));
                                viewProfile(context, requestBody);
                            } else {
                                getView().onErrorToast(context.getString(R.string.msg_unable_connect_server));
                                getView().enableLoadingBar(context, false, "");
                            }*/
