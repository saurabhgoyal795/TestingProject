...
public class MobileRechargePresenter extends BasePresenter<MobileRechargeListner> {

    private static int retryCount = 0;

    public void mobileRecharge(final Context context, final HashMap<String, RequestBody> requestBody) {

        getView().enableLoadingBar(context, true, context.getString(R.string.txt_please_wait));

        MyApplication.getInstance().getAPIInterface().topupRecharge(requestBody)
